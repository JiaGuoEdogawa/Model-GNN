...
Thie is the main function for training the Vanilla-GNN to maximize EE.
...
import sys
sys.path.append('../')
from common_funcs.PENN_model import penn_2d
...
LEARNING_RATE = config['LEARNING_RATE'] = 4e-3
MAX_EPOCHS = config['MAX_EPOCHS'] = 200
...
n_hidden = [256] * 2 + [2]
y_pred = penn_2d(x, nTX_allUE, nTX_allBS, n_hidden, output_activation=tf.nn.tanh, is_BN=True, is_trainBN=is_trainBN, is_lst_transfer=False)
y_pred = y_pred / (tf.sqrt(tf.reduce_sum(tf.pow(y_pred, 2), axis=[-1, -2, -3], keepdims=True)) + 1e-6)
...
# ============================================== TRAIN INITIALIZE ==================================================== #
init, saver, sess, config_train = tf_initializer(gpu_fraction=0.999)
